...
async function changeToRevealing(params) {
  let ongoingMarkets = await app.model.Market.findAll({
    condition: {
...
async function changeAnnouncingToFinished(params) {
  let announcingMarkets = await app.model.Market.findAll({
    condition: {
...
module.exports = async function () {
  ...
  await app.sdb.load('Market', ['state', 'revealHeight', 'total', 'id'], ['id'])
  await app.sdb.load('Share', ['share', 'mid', 'address', 'choice'], [['mid', 'address', 'choice']])
  await app.sdb.load('Result', ['share', 'mid', 'choice'], [['mid', 'choice']])
